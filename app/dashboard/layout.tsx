...
function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-12 left-0 hidden h-[0.5px] w-full bg-gradient-to-r from-transparent from-[-10%] via-zinc-700 via-50% to-transparent to-110% lg:block 2xl:top-16"></div>
      <div className="relative flex hidden w-full justify-center p-2 lg:block">
        {isLoading ? (
          <AllCryptosSkeleton />
        ) : (
          <div className="lg:mt-14 2xl:mt-20">
            <AllCryptos />
            <Paginate />
          </div>
        )}
